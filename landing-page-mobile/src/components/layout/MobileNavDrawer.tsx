...
const MobileNavDrawer: React.FC<MobileNavDrawerProps> = ({ isOpen, onClose }) => {
  ...
  // Exit: animate then notify parent
  const handleClose = () => {
    if (isClosing) return;
    setIsClosing(true);
  ...
  return (
    <>
      {/* Full-screen darkening overlay (covers entire screen; drawer sits on top) */}
      <div
        className={`fixed inset-0 bg-black/50 z-[100] transition-opacity duration-300 ease-out ${
          isClosing ? 'opacity-0' : 'opacity-100'
        }`}
        aria-hidden
        onClick={handleClose}
      />
      {/* Drawer from the right, slides in/out */}
      <div
        className={`fixed top-0 right-0 h-full w-[311px] max-w-[85vw] bg-bg-category flex flex-col items-stretch pt-20 px-5 pb-5 z-[101] overflow-y-auto shadow-[-4px_0_24px_rgba(0,0,0,0.12)] transition-transform duration-300 ease-out ${
      ...
      >
        ...
        <div className="flex flex-col gap-6 flex-1">
          ...
          {navLinks.map((link) => (
            ...
            >
              {link.label === 'Your cart' && (
                <img src="/images/icons/cart-icon.svg" alt="" className="w-5 h-5 flex-shrink-0" aria-hidden />
              )}
