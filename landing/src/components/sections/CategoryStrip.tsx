...
const CategoryStrip: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-full mx-auto flex items-center gap-2 min-h-[130px] md:min-h-0 md:justify-between md:gap-5">
      <button type="button" className="flex-shrink-0 w-12 h-12 flex items-center justify-center text-text-brown md:hidden" aria-label="Scroll left" onClick={() => scroll('left')}>
        <img src="/images/icons/dropdown-icon.svg" alt="" className="w-10 h-10 rotate-90" />
      </button>
      <div
        ref={scrollRef}
        className="flex-1 min-w-0 w-full overflow-x-auto overflow-y-hidden flex items-start gap-5 px-2 scrollbar-hide touch-pan-x md:overflow-visible md:flex md:items-center md:justify-between"
        style={{ scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch' }}
      >
        {CATEGORIES.map((cat) => (
          <div key={cat.id} style={{ scrollSnapAlign: 'start' }} className="flex-shrink-0 md:inline-block">
            <a href={`#${cat.id}`} className="md:block" onClick={(e) => { e.preventDefault(); setSelectedId(cat.id); }}>
              <CategoryCard name={cat.label} icon={cat.icon} selected={selectedId === cat.id} />
            </a>
          </div>
        ))}
      </div>
      <button type="button" className="flex-shrink-0 w-12 h-12 flex items-center justify-center text-text-brown md:hidden" aria-label="Scroll right" onClick={() => scroll('right')}>
        <img src="/images/icons/dropdown-icon.svg" alt="" className="w-10 h-10 -rotate-90" />
      </button>
    </div>
  )
