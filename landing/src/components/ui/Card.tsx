...
const Card: React.FC<CardProps> = ({ children, className = '', variant = 'default' }) => {
  const variantClasses = {
    ...
    product: 'rounded-[20px] md:rounded-card shadow-[0px_4px_8px_-2px_rgba(126,141,120,0.01),0px_2px_4px_-2px_rgba(0,0,0,0.03)] bg-bg-product',
  }
  return <div className={`${variantClasses[variant]} ${className}`.trim().replace(/\s+/g, ' ')}>{children}</div>
}
