...
const SearchBar: React.FC<SearchBarProps> = ({ placeholder = 'Search', onSearch, className = '' }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className={`flex-1 max-w-[440px] min-w-[200px] ${className}`}>
      <div className="self-stretch rounded-full bg-bg-primary border border-text-border flex items-center px-3 py-2">
        <img src="/images/icons/search-icon.svg" alt="Search" className="w-6 h-6 flex-shrink-0" />
        <input
          type="text"
          value={query}
